...
/**
@author Vidhi Shah
This class is responsible for graphically visualizing the tree.
This is implemented via swing framework, by calculating each node's x & y coordinates according to their intern-relationship with one another (i.e, parent, left, right, etc), and then just drawing ovals and lines based on these coordinates in-order to represent the nodes and edges.
*/
...
public class TreePrinter <T extends Comparable<T>> extends JPanel 
{
	...
	*/
	...
	g Graphics object used to draw nodes and arcs. 
	*/
	...
	/*
	Save local logical position of each node according to a base position of root. Then shift node left by offset so that tree would be displayed starting at x = 0. 
	*/
	...
	coordinates - the base coordinates positions for this node.
	g - the Graphic object which could draw the node. 
	*/
	...
	The wires are painted in red to distinguish them from the real pointers, which are painted in green. 
	node - the current node to be printed.
	coordinates - the base coordinates positions for this node.
	g - the Graphic object which could draw the node. 
	*/
