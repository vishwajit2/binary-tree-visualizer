...
/**
 * @author Vidhi Shah This class is responsible for graphically visualizing the
 *         tree. This is implemented via swing framework, by calculating each
 *         node's x & y coordinates according to their intern-relationship with
 *         one another (i.e, parent, left, right, etc), and then just drawing
 *         ovals and lines based on these coordinates in-order to represent the
 *         nodes and edges.
 */
...
public class TreePrinter<T extends Comparable<T>> extends JPanel {
	...
	// the wired binary search tree to display.
	...
	/*
	 * paintComponent: Override generic paint method to call concrete drawing one.
	 */
	@Override
	...
	/*
	 * printTree: Responsible for the painting of tree. g Graphics object used to
	 * draw nodes and arcs.
	 */
	public void printTree(Graphics g) {
		...
		if (tree.getRoot() != null) {
			...
			// Iterate the table and print each node's shape, data & arc according to it's
			// position:
	...
	/*
	 * Save local logical position of each node according to a base position of
	 * root. Then shift node left by offset so that tree would be displayed starting
	 * at x = 0.
	 */
	private void calculateCoordinates() {
		...
		// save left most point in-order to shift left all points later by it's distance
		// from x = 0:
		this.globalHorizontalOffset = leftMostPoint;
	}

	/*
	 * Paint an individual node frame and data. node - the current node to be
	 * printed. coordinates - the base coordinates positions for this node. g - the
	 * Graphic object which could draw the node.
	 */
	...
	/*
	 * Paint an individual node's arcs to his children. If the boolean flag was set
	 * on, also non-null wires would be painted, i.e, all wires other than minimum's
	 * left & maximum's right. The wires are painted in red to distinguish them from
	 * the real pointers, which are painted in green. node - the current node to be
	 * printed. coordinates - the base coordinates positions for this node. g - the
	 * Graphic object which could draw the node.
	 */
