 ...
 * 1)supplying a GUI interactive application 
 * which supports the wired binary search tree operations & queries.
 * 2)It also supports a print visualization of the existing tree. 
 * 3) It is support importing an input file with  following commands....
 * 
 * commands        |
 * insert ---> <numeric id> required
 * delete----> <numeric id> required
 * search-----> <numeric id> required			 
 * minimum										 
 * maximum										 	
 * median										 
 * predecessor-->	<numeric id> required					 
 * successor--->	<numeric id> required					 
 * inorder										 
 * preorder										 
 * postorder									 
 * ----------------------------------------------
...
public class GUIApplication extends JFrame implements ActionListener
{
	...
	//for  Fonts: 
	private static final Font defaultFont = new Font("Tahoma", 0, 14);//"Tahoma" is font used for presenting infomation... 
	...
	/**
	 * This class responding for
	 * 1) Initialize all GUI components:
	 *2)the wired binary tree to manipulate and display it: 
...
/*------------------------------------------------------------------------------------------------------------------------------*/
		...
		treeWalksPanel.add(preOrderButton); //Adding......
...
/**********************************************************************************************************************/
	...
	/**
	 *  GUI event handler:
     *  1)handles all push buttons and check box: 
...
/*----------------------------------------------------------------------------------------------------------------------------*/
		...
		else if (triggeringObject == deleteButton)
		{
			...
			if (node == null)
				displayMessage(("Deleteion failed: ID/MIS" + inputID + " does not exist on tree"), JOptionPane.ERROR_MESSAGE);
			else 
			{
...
/*-----------------------------------------------------------------------------------------------------------------------------*/   
		...
		else if (triggeringObject == searchButton)
		{
			...
			if (node == null)
				displayMessage(("Search failed: ID/MIS " + inputID + " does not exist on this tree"), JOptionPane.INFORMATION_MESSAGE);
			else 
			{
...
/*----------------------------------------------------------------------------------------------------------------------*/
		...
		else if (triggeringObject == successorButton)
		{
			...
			else 
			{
				...
				else displayMessage((String.format("<%d> is the maximum node... it has no successor.", inputID)), JOptionPane.INFORMATION_MESSAGE);
			}	
			return;
		}
...
/*---********************************************************************************************************************---*/
	...
	private boolean inputIsValid(ActionEvent event)
	{
		...
		if (inputID.isEmpty())
		{
			...
			{
				displayMessage(String.format("Student ID/MIS is a mandatory field for this operation"), JOptionPane.ERROR_MESSAGE);
				return false;
			}
		}
		else // make sure student id/mis  is positive integer:
		{
			...
			{	
				...
				if (value <= 0)
				{
					message = String.format("Invalid student id/mis input: \"%s\", ID/MIS must be a positive number", inputID);
					displayMessage(message, JOptionPane.ERROR_MESSAGE);
					return false;
				}
			}
